...
def ConvertRunClone(Run,Clone,TrajNumber,Conf1,DataDir,RunList,CloneList,NumGensList,Usetrjcat=False,trjcatFlags=["-cat"],DiscardFirstN=0,Stride=1,WhichRunsClones=None,NamingConvention=0,OutDir="./Trajectories/",InFilenameRoot="frame",OutFilenameRoot="trj",OutFileType=".lh5",MinGen=0,MaxGen=100000,AtomIndices=None,DiscardHighRMSD=True,MaxRMSD=7,PDBFilename=None,CenterConformations=True,SkipTrajCat=False,RemovePBC=False,IndexGroupToWrite=0):
    ...
    if not os.path.exists(OutFilename):
        if SkipTrajCat==False:
            if Usetrjcat==False:
                Traj=Trajectory.Trajectory.LoadFromXTC(FilenameList,Conf=Conf1)
            else:
                CMD="trjcat -f %s %s"%(string.join(FilenameList),string.join(trjcatFlags))
                os.system(CMD)
                if RemovePBC==True:
                    tprfile=glob.glob(InFilenameRoot+'/RUN%d/CLONE%d/frame0.tpr'%(Run,Clone))
                    if len(tprfile)>0:
                        CMD="echo %d | trjconv -f trajout.xtc -s %s -pbc mol"%(IndexGroupToWrite,tprfile[0])
                        print "Running command %s to strip output"%(CMD)
                        os.system(CMD)
                    else:
                        print 'Warning:  could not find TPR file '+InFilenameRoot+'/RUN%d/CLONE%d/frame0.tpr'%(Run,Clone)+'.  Could not remove PBC.'
